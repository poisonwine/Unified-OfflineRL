...
class SQLPolicy(BasePolicy):
    """
    paper:Offline RL with No OOD Actions: In-Sample Learning via Implicit Value Regularization
    ICLR 2023
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        sp_term = (q-v) / (self._alpha) + 0.5
        weight = torch.where(sp_term > 0, 1.0, 0.)
        loss_v = weight * (sp_term**2)
        value_loss = (loss_v + v / self._alpha).mean()
        ...
        with torch.no_grad():
            ...
            weight = q -v
            # weight = torch.maximum(weight, torch.tensor([0]).to(torch.device(self.device)))
            weight = torch.clip(weight, 0.0, 100.0)
